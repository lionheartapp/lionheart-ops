...
def format_ascii_box(design_system: dict) -> str:
    ...
    lines.append("+" + "-" * w + "+")
    lines.append(f"|  TARGET: {project} - RECOMMENDED DESIGN SYSTEM".ljust(w) + "|")
    lines.append("+" + "-" * w + "+")
    lines.append("|" + " " * w + "|")

    # Pattern section
    lines.append(f"|  PATTERN: {pattern.get('name', '')}".ljust(w) + "|")
    if pattern.get('conversion'):
        lines.append(f"|     Conversion: {pattern.get('conversion', '')}".ljust(w) + "|")
    if pattern.get('cta_placement'):
        lines.append(f"|     CTA: {pattern.get('cta_placement', '')}".ljust(w) + "|")
    lines.append("|     Sections:".ljust(w) + "|")
    for i, section in enumerate(sections, 1):
        lines.append(f"|       {i}. {section}".ljust(w) + "|")
    lines.append("|" + " " * w + "|")

    # Style section
    lines.append(f"|  STYLE: {style.get('name', '')}".ljust(w) + "|")
    ...
    if style.get("performance") or style.get("accessibility"):
        ...
        lines.append(f"|     {perf_a11y}".ljust(w) + "|")
    lines.append("|" + " " * w + "|")

    # Colors section
    lines.append("|  COLORS:".ljust(w) + "|")
    lines.append(f"|     Primary:    {colors.get('primary', '')}".ljust(w) + "|")
    lines.append(f"|     Secondary:  {colors.get('secondary', '')}".ljust(w) + "|")
    lines.append(f"|     CTA:        {colors.get('cta', '')}".ljust(w) + "|")
    lines.append(f"|     Background: {colors.get('background', '')}".ljust(w) + "|")
    lines.append(f"|     Text:       {colors.get('text', '')}".ljust(w) + "|")
    if colors.get("notes"):
    ...
    # Typography section
    lines.append(f"|  TYPOGRAPHY: {typography.get('heading', '')} / {typography.get('body', '')}".ljust(w) + "|")
    ...
    if typography.get("google_fonts_url"):
        lines.append(f"|     Google Fonts: {typography.get('google_fonts_url', '')}".ljust(w) + "|")
    if typography.get("css_import"):
        lines.append(f"|     CSS Import: {typography.get('css_import', '')[:70]}...".ljust(w) + "|")
    lines.append("|" + " " * w + "|")

    # Key Effects section
    if effects:
        lines.append("|  KEY EFFECTS:".ljust(w) + "|")
    ...
    if anti_patterns:
        lines.append("|  AVOID (Anti-patterns):".ljust(w) + "|")
        for line in wrap_text(anti_patterns, "|     ", BOX_WIDTH):
            lines.append(line.ljust(w) + "|")
        lines.append("|" + " " * w + "|")

    # Pre-Delivery Checklist section
    lines.append("|  PRE-DELIVERY CHECKLIST:".ljust(w) + "|")
    checklist_items = [
