...
export async function POST(
  ...
  try {
    ...
    return await runWithOrgContext(orgId, async () => {
      const updated = parsed.data.action === 'add'
        ? await addResponder(orgId, id, ctx.userId, parsed.data.responderId)
        : await removeResponder(orgId, id, ctx.userId, parsed.data.responderId)
    ...
    console.error('POST /api/it/incidents/[id]/responders error:', error)
