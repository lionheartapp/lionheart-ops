...
export const POST = withAuth<z.infer<typeof BulkActionSchema>>(async ({ body }) => {
  ...
  if (action === 'assign') {
    ...
    const result = await (prisma.iTTicket.updateMany as Function)({
...
}, { permission: PERMISSIONS.IT_TICKET_ASSIGN, schema: BulkActionSchema })
