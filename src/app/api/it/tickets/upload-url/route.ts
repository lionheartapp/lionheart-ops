/**
 * POST /api/it/tickets/upload-url — generate a signed Supabase Storage upload URL
 *
 * Returns a signed URL for client-side direct upload to 'maintenance-photos' bucket.
 * Reuses the same bucket as maintenance tickets since the upload pattern is identical.
 */
...
export const POST = withAuth(async ({ req, orgId }) => {
  ...
  const storagePath = `${orgId}/it/${Date.now()}-${sanitizeFileName(fileName)}`
  ...
  try {
    ...
    if (error || !data) {
      logger.error({ error: String(error) }, 'Supabase upload URL failed (IT)')
...
}, { permission: PERMISSIONS.IT_TICKET_SUBMIT })
