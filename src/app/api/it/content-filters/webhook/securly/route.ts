...
export async function POST(req: NextRequest) {
  try {
    ...
    const signature = req.headers.get('x-securly-signature') || ''

    const config = await (rawPrisma.iTContentFilterConfig as unknown as PrismaDelegate).findUnique({
      where: { organizationId_provider: { organizationId: orgId, provider: 'SECURLY' } },
    })
    ...
    const normalized = transformSecurlyPayload(payload)
    await processFilterEvent(orgId, 'SECURLY', normalized)

    return NextResponse.json(ok({ received: true }))
  } catch (error) {
    logger.error({ error: String(error) }, 'Securly webhook processing failed')
