...
export const GET = withAuth(async ({ ctx, searchParams }) => {
  ...
  const filters: Partial<ListTicketsInput> = { limit, skip, status: status as ListTicketsInput['status'], category: category as ListTicketsInput['category'], priority: priority as ListTicketsInput['priority'], assignedToId, schoolId, search }
