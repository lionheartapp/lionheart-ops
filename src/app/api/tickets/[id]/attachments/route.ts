...
export async function GET(req: NextRequest, { params }: RouteParams) {
  const log = logger.child({ route: '/api/tickets/[id]/attachments', method: 'GET' })
  try {
    ...
    log.error({ err: error }, 'Failed to fetch ticket attachments')
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  const log = logger.child({ route: '/api/tickets/[id]/attachments', method: 'POST' })
  try {
    const { id } = await params
    const orgId = getOrgIdFromRequest(req)
    Sentry.setTag('org_id', orgId)
    ...
    log.error({ err: error }, 'Failed to create ticket attachment')
    Sentry.captureException(error)
    return NextResponse.json(
      fail('INTERNAL_ERROR', error instanceof Error ? error.message : 'Internal server error'),
      { status: 500 }
