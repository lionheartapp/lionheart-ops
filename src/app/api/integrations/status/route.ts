...
export async function GET(req: NextRequest) {
  try {
    ...
    return await runWithOrgContext(orgId, async () => {
      // Per-user cache with org tag — payload mixes per-user (gcal/mscal) and
      // org-level (pco/twilio) creds, so either side's mutation can blast this
      // user's entry via `invalidateOrgCache(orgId, 'integrations')` or
      // `invalidateUserCache(userId, 'integrations')`.
      const payload = await cachePerUser(
        ctx.userId,
        `integrations:status:org=${orgId}`,
        async () => {
