...
function getResultBadge(game: Game): { label: string; className: string } | null {
  ...
  if (tied) return { label: `T ${game.homeScore}-${game.awayScore}`, className: 'bg-stone-100 text-stone-700' }
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-3">
          ...
          <div className="w-40 h-5 bg-stone-200 rounded" />
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-xl font-semibold text-stone-700 mb-2">Schedule Not Found</h1>
          <p className="text-sm text-stone-500">{error || 'This athletics schedule could not be found.'}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">Athletics Schedule</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            {filteredUpcoming.length === 0 ? (
              <div className="rounded-xl border border-stone-200 bg-white p-6 text-center">
                <p className="text-sm text-stone-500">No upcoming games scheduled</p>
              </div>
            ) : (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-xs text-stone-400">{formatDate(game.startTime)}</span>
                            {result && (
          ...
          <div className="space-y-6">
            ...
            {currentStandings && currentStandings.length > 0 && (
              <div>
                <h2 className="text-sm font-semibold text-stone-500 uppercase tracking-wider mb-3">Standings</h2>
                <div className="rounded-xl border border-stone-200 bg-white overflow-hidden">
                  <table className="min-w-full divide-y divide-stone-100">
                    <thead>
                      <tr className="bg-stone-50/50">
                        <th className="px-3 py-2 text-left text-xs font-semibold text-stone-500">Team</th>
                        <th className="px-2 py-2 text-center text-xs font-semibold text-stone-500">W</th>
                        <th className="px-2 py-2 text-center text-xs font-semibold text-stone-500">L</th>
                        <th className="px-2 py-2 text-center text-xs font-semibold text-stone-500">T</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-50">
                      {currentStandings.map((s, i) => (
                        <tr key={i}>
                          ...
                          <td className="px-2 py-2 text-sm text-center text-stone-500">{s.ties}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {data.calendarTeams.length > 0 && (
              <div>
                ...
                <div className="rounded-xl border border-stone-200 bg-white divide-y divide-stone-50">
                  {data.calendarTeams
                    ...
                    .map((ct) => (
                      <div key={ct.calendarId} className="flex items-center justify-between px-4 py-3">
                        <div>
                          ...
                          <div className="text-xs text-stone-400">{ct.sportName}</div>
                        </div>
                        <button
                          type="button"
