    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex items-center gap-2 text-slate-400 mb-2"><DollarSign size={16} /> Total Revenue</div>
          <p className="text-2xl font-bold">${(totalRevenue / 100).toFixed(2)}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex items-center gap-2 text-slate-400 mb-2"><CreditCard size={16} /> Transactions</div>
          <p className="text-2xl font-bold">{total}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex items-center gap-2 text-slate-400 mb-2"><TrendingUp size={16} /> Successful</div>
          <p className="text-2xl font-bold">{payments.filter(p => p.status === 'SUCCEEDED').length}</p>
        </div>
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-800"><h2 className="font-semibold">Recent Payments</h2></div>
            ...
            {loading ? (
              ...
              payments.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="px-5 py-3 text-slate-400">{p.subscription?.plan?.name || '—'}</td>
                  ...
                  <td className="px-5 py-3 hidden sm:table-cell text-slate-400">{new Date(p.createdAt).toLocaleDateString()}</td>
                </tr>
              ))
            )}
          </tbody>
