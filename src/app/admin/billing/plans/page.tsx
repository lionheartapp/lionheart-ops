        ...
        {plans.map((plan) => (
          <div key={plan.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${plan.isActive ? 'bg-green-500/10 text-green-400' : 'bg-slate-700 text-slate-300'}`}>
            ...
            {plan.trialDays > 0 && (
              <p className="text-sm text-slate-400 mb-3">{plan.trialDays}-day free trial</p>
            )}
              ...
              <div className="space-y-1 text-sm">
                {plan.features && typeof plan.features === 'object' && Object.entries(plan.features as Record<string, unknown>).map(([key, val]) => (
                  <div key={key} className="flex items-center gap-2 text-slate-400">
                    {val === true ? <Check size={14} className="text-green-400" /> : val === false ? <X size={14} className="text-slate-600" /> : null}
                    <span>{key}: {typeof val === 'boolean' ? '' : val === -1 ? 'Unlimited' : String(val)}</span>
                  </div>
                ))}
              </div>
            </div>
            <p className="text-xs text-slate-500 mt-3">{plan._count?.subscriptions || 0} subscribers</p>
          </div>
        ))}
      </div>
    </div>
