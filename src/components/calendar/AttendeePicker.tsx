...
function Avatar({ person, size = 'sm' }: { person: { firstName?: string | null; email?: string; avatar?: string | null }; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${dim} rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 overflow-hidden`}>
      {person.avatar ? (
        <img src={person.avatar} alt="" className="w-full h-full rounded-full object-cover" />
      ) : (
        <span className={`${txt} font-medium text-gray-500`}>
    ...
    <div ref={containerRef} className="relative">
      ...
      {value.length > 0 && (
        <div className={`flex flex-wrap gap-1.5 ${compact ? 'mb-1.5' : 'mb-2'}`}>
          {value.map((attendee) => (
            <div key={attendee.id} className="flex items-center gap-1.5 pl-1 pr-1.5 py-0.5 rounded-full bg-gray-100 text-xs">
              <Avatar person={attendee} size="sm" />
              <span className="text-gray-700 truncate max-w-[120px]">{attendee.firstName || attendee.email.split('@')[0]}</span>
              <button type="button" onClick={() => onChange(value.filter((a) => a.id !== attendee.id))} className="p-0.5 rounded-full hover:bg-gray-200 transition-colors" aria-label={`Remove ${getFullName(attendee)}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full pl-9 pr-3 text-sm bg-white border border-gray-300 rounded-lg focus:outline-none focus-visible:ring-1 focus-visible:ring-gray-900/10 focus:border-gray-900 placeholder-gray-400 ${compact ? 'py-2' : 'py-3'}`}
          role="combobox" aria-expanded={showResults} aria-haspopup="listbox" aria-autocomplete="list"
        />
      </div>
      {showResults && debouncedQuery.length >= 2 && (
        <div role="listbox" className="absolute top-full left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-white rounded-xl shadow-lg border border-gray-200 z-50 py-1">
          {isLoading && <div className="px-3 py-2 text-xs text-gray-400">Searching...</div>}
          {!isLoading && filteredResults.length === 0 && <div className="px-3 py-2 text-xs text-gray-400">No results found</div>}
          {filteredResults.map((result) => (
            <button key={result.id} type="button" role="option" onClick={() => handleSelect(result)} className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-gray-50 transition-colors">
              <Avatar person={result} size="md" />
              <div className="min-w-0 flex-1">
                <div className="text-sm text-gray-900 truncate">{result.firstName && result.lastName ? `${result.firstName} ${result.lastName}` : result.firstName || result.email}</div>
                {result.jobTitle && <div className="text-xs text-gray-400 truncate">{result.jobTitle}</div>}
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className={`w-full flex items-center justify-between px-3 text-sm bg-white border border-gray-300 rounded-lg hover:border-gray-400 focus:outline-none focus-visible:ring-1 focus-visible:ring-gray-900/10 transition-colors ${compact ? 'py-2' : 'py-3'}`}
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
          ...
          <div className="p-2 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus-visible:ring-1 focus-visible:ring-gray-900/10 focus:border-gray-900 placeholder-gray-400"
              />
            </div>
          </div>
          ...
          <div role="listbox" className="max-h-52 overflow-y-auto py-1">
            {isLoading && <div className="px-3 py-3 text-xs text-gray-400 text-center">Loading...</div>}
            ...
            {filtered.map((person) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {person.jobTitle && <div className="text-xs text-gray-400 truncate">{person.jobTitle}</div>}
                </div>
              </button>
            ))}
          </div>
