    ...
    <div className="space-y-6">
              ...
              <div className="space-y-1">
                {teams.filter((t) => t.id !== team.id).map((opponent) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-sm text-stone-600">vs {opponent.name}</span>
                      {hasResult ? (
      ...
      <div className="overflow-x-auto -mx-4 px-4 hidden sm:block">
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr>
              <th className="px-3 py-2 text-left text-xs font-medium text-stone-500 bg-stone-50 border border-stone-200 sticky left-0 z-10">
                Team
              </th>
              {teams.map((t) => (
                <th
                  key={t.id}
                  className="px-3 py-2 text-center text-xs font-medium text-stone-500 bg-stone-50 border border-stone-200 min-w-[100px]"
                >
          ...
          <tbody>
            {teams.map((rowTeam) => (
              <tr key={rowTeam.id}>
                <td className="px-3 py-2.5 font-medium text-slate-900 bg-white border border-stone-200 sticky left-0 z-10 whitespace-nowrap">
                  <div className="flex items-center gap-2">
                ...
                {teams.map((colTeam) => {
                  if (rowTeam.id === colTeam.id) {
                    return (
                      <td key={colTeam.id} className="px-3 py-2.5 text-center bg-stone-100 border border-stone-200">
                  ...
                  if (!match) {
                    return (
                      <td key={colTeam.id} className="px-3 py-2.5 text-center border border-stone-200 text-stone-300">
                        &mdash;
                      </td>
                    )
                  }
                  ...
                  return (
                    <td
                      key={colTeam.id}
                      onClick={() => onMatchClick(match)}
                      className={`px-3 py-2.5 text-center border border-stone-200 cursor-pointer transition-colors ${
                        isWinner
      ...
      {/* Standings */}
      <div>
        <h4 className="text-sm font-semibold text-stone-700 mb-2">Standings</h4>
        <div className="ui-glass-table">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-stone-50">
                <th className="px-3 py-2 text-left text-xs font-medium text-stone-500">#</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-stone-500">Team</th>
                <th className="px-3 py-2 text-center text-xs font-medium text-stone-500">W</th>
                <th className="px-3 py-2 text-center text-xs font-medium text-stone-500">L</th>
                <th className="px-3 py-2 text-center text-xs font-medium text-stone-500">GP</th>
              </tr>
            </thead>
            <tbody>
              {sortedTeams.map((team, idx) => {
                ...
                return (
                  <tr key={team.id} className="border-t border-stone-100">
                    <td className="px-3 py-2 text-stone-400 font-medium">{idx + 1}</td>
                    ...
                    <td className="px-3 py-2 text-center text-stone-500">{s.played}</td>
                  </tr>
                )
              })}
            </tbody>
