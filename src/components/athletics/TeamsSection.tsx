...
const GRADE_STYLES: Record<string, string> = {
  ...
  MIDDLE_SCHOOL: 'bg-slate-100 text-slate-600',
              ...
              return (
                <div key={team.id} className="ui-glass-hover p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500 mt-0.5">{team.sport.name}</div>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${LEVEL_STYLES[team.level] || 'bg-slate-100 text-slate-600'}`}>
                      ...
                      {rec && (
                        <span className="text-xs font-medium text-slate-600">
                          ...
                          {rec.ties > 0 && <>-<span className="text-slate-500">{rec.ties}</span></>}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Team</th>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Level</th>
                  <th className="text-left px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wider hidden md:table-cell">Coach</th>
                  <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wider hidden md:table-cell">Record</th>
                  <th className="text-center px-4 py-3 font-medium text-slate-500 text-xs uppercase tracking-wider hidden md:table-cell">G/P</th>
                ...
                {displayTeams.map((team) => {
                  ...
                  return (
                    <tr key={team.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600">{team.sport.name}</td>
                      <td className="px-4 py-3 text-slate-500">{team.season.name}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 flex-wrap">
                          {team.gradeLevel && (
                            <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${GRADE_STYLES[team.gradeLevel] || 'bg-slate-100 text-slate-600'}`}>
