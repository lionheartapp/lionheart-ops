    ...
    <div className="space-y-4">
      ...
      {showToolbar && (
        <div className="flex items-center gap-3">
          <div className="h-[46px] w-64 bg-slate-100 rounded-lg animate-pulse" />
          <div className="h-[46px] w-32 bg-slate-100 rounded-lg animate-pulse" />
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        {Array.from({ length: rows }).map((_, rowIdx) => (
          <div
            key={rowIdx}
            className="flex items-center gap-4 px-4 py-3.5 border-b border-slate-50 last:border-b-0"
          >
            {Array.from({ length: columns }).map((_, colIdx) => (
              <div key={colIdx} className="flex items-center gap-2" style={{ width: `${colIdx === 0 ? 28 : 12 + Math.random() * 10}%` }}>
                {colIdx === 0 && (
                  <div className="w-3 h-3 rounded-full bg-slate-100 animate-pulse flex-shrink-0" />
                )}
                <div
...
export function ScheduleSkeleton({ groups = 3 }: { groups?: number }) {
  return (
    <div className="space-y-4">
      {/* Toolbar skeleton */}
      <div className="flex items-center gap-3">
        <div className="h-[46px] w-56 bg-slate-100 rounded-lg animate-pulse" />
      ...
      {Array.from({ length: groups }).map((_, groupIdx) => (
        <div key={groupIdx} className="space-y-2">
          {/* Date header */}
          <div className="h-4 w-40 bg-slate-200/60 rounded animate-pulse" style={{ animationDelay: `${groupIdx * 100}ms` }} />
          ...
          {Array.from({ length: 2 + Math.floor(Math.random() * 2) }).map((_, cardIdx) => (
            ...
            >
              <div className="w-1 h-10 rounded-full bg-slate-100 animate-pulse flex-shrink-0" />
              <div className="flex-1 space-y-2">
                <div className="h-3.5 w-3/5 bg-slate-100 rounded animate-pulse" style={{ animationDelay: `${(groupIdx * 3 + cardIdx) * 75}ms` }} />
                <div className="h-3 w-2/5 bg-slate-50 rounded animate-pulse" style={{ animationDelay: `${(groupIdx * 3 + cardIdx) * 75 + 50}ms` }} />
              </div>
              <div className="h-6 w-14 bg-slate-50 rounded-full animate-pulse" />
            </div>
          ))}
        </div>
      ))}
