              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Labor Entries</h4>
                    {laborEntries.length > 0 && (
                      <span className="text-xs text-gray-400">({laborEntries.length})</span>
                    )}
                  </div>
                  <button
                ...
                <AnimatePresence>
                  {showLaborForm && (
                    <motion.div
                      ...
                      className="overflow-hidden"
                    >
                      <div className="bg-gray-50/80 border border-gray-200 rounded-xl px-4 pb-4 mb-3">
                        <LaborEntryForm
                          ticketId={ticketId}
                ...
                {laborLoading ? (
                  ...
                  <p className="text-xs text-gray-400 py-2">No labor entries yet. Use the timer or add a manual entry.</p>
                ) : (
                  <div className="space-y-2">
                    {laborEntries.map((entry) => {
                      ...
                      return (
                        <div
                          key={entry.id}
                          className="flex items-start justify-between gap-3 p-3 bg-gray-50/80 rounded-xl border border-gray-100"
                        >
                          <div className="min-w-0 flex-1">
                            ...
                            {entry.notes && (
                              <p className="text-[11px] text-gray-500 mt-0.5 truncate">{entry.notes}</p>
                            )}
                          </div>
                          <button
                            onClick={() => deleteLaborMutation.mutate(entry.id)}
                            disabled={isDeleting}
                            className="text-gray-300 hover:text-red-400 transition-colors cursor-pointer disabled:opacity-50 flex-shrink-0 p-0.5"
                            title="Delete entry"
                          >
                            {isDeleting ? (
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Cost Entries</h4>
                    {costEntries.length > 0 && (
                      <span className="text-xs text-gray-400">({costEntries.length})</span>
                    )}
                  </div>
                  <button
                ...
                {costsLoading ? (
                  ...
                  <p className="text-xs text-gray-400 py-2">No cost entries yet. Add materials, parts, or vendor expenses.</p>
                ) : (
                  <div className="space-y-2">
                        ...
                        >
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                {entry.vendor && (
                                  <span className="text-xs font-medium text-gray-800">{entry.vendor}</span>
                                )}
                                <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-medium">
                                  {formatMoney(entry.amount)}
                                </span>
                              </div>
                              <p className="text-[11px] text-gray-600 mt-0.5 truncate">{entry.description}</p>
