...
export default function StepAsset({
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-base font-semibold text-gray-900 mb-1">Which asset needs attention?</h3>
        <p className="text-sm text-gray-500">Link this ticket to specific equipment (optional)</p>
      </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-mono font-semibold text-emerald-700 mb-0.5">{assetLabel}</p>
                <p className="text-sm font-medium text-gray-900 truncate">{assetLabel.split(' — ')[1] || assetLabel}</p>
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="relative">
          ...
          <AnimatePresence>
                ...
                {results.map((asset) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 hover:bg-emerald-50 transition-colors cursor-pointer border-b border-gray-50 last:border-0"
                    >
                      <div className="flex items-center gap-2">
                        <Package className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" />
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-mono font-semibold text-emerald-700">{asset.assetNumber}</span>
                            {asset.category && (
            ...
            {isDropdownOpen && query.trim() && results.length === 0 && !isLoading && (
              ...
              >
                <p className="text-sm text-gray-500">No assets found for &ldquo;{query}&rdquo;</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      {!hasSelection && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-xs text-gray-400 mb-2">Or enter asset number directly:</p>
          <div className="flex gap-2">
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden flex-1">
              <span className="px-2 py-2 text-xs font-mono text-gray-400 bg-gray-50 border-r border-gray-200">AST-</span>
              <input
                type="text"
                value={manualNumber}
                onChange={(e) => {
                ...
                placeholder="0001"
                className="flex-1 px-3 py-2 text-sm text-gray-900 bg-white focus:outline-none font-mono"
              />
            </div>
            <button
              type="button"
              ...
              className="px-3 py-2 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50"
            >
