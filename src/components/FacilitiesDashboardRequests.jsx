...
import {
  filterNewFacilitiesRequests,
  filterInProgressFacilitiesRequests,
  filterThisWeeksFacilitiesRequests,
...
function RequestCard({ request, onClick, priorityStyles, draggable, onDragStart, ticketPrefix = 'FAC' }) {
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mt-2">
        <span className="text-xs text-zinc-500 dark:text-zinc-400">
          {ticketPrefix}-{String(request.id).padStart(4, '0')}
        </span>
...
export default function FacilitiesDashboardRequests({
  ...
  const myTickets = filterRequestsByAssignee(requests, currentUser?.name, 'Facilities')
  const thisWeeks = filterThisWeeksFacilitiesRequests(myTickets)
  const toDoRequests = filterNewFacilitiesRequests(thisWeeks)
  const inProgressRequests = filterInProgressFacilitiesRequests(thisWeeks)
  ...
  const createCardDropHandlers = (column, request, index) => ({
    ...
    onDragLeave: (e) => {
      e.stopPropagation()
      if (!e.currentTarget.contains(e.relatedTarget)) setDropTargetBeforeId(null)
    },
    onDrop: (e) => {
      e.stopPropagation()
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-zinc-200 dark:divide-zinc-700 min-h-[200px]">
        <div
          className={`p-4 flex flex-col flex-1 min-h-[160px] rounded-lg border-2 border-dashed transition-colors ${
            dropTargetColumn === 'to-do' ? 'border-blue-500 bg-blue-500/10 dark:bg-blue-500/20' : 'border-transparent'
          }`}
          {...toDoDropHandlers}
        >
          <h3 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3">To Do</h3>
          <div className="flex-1 space-y-2 overflow-y-auto">
            {toDoRequests.length === 0 ? (
              <p className="text-sm text-zinc-500 dark:text-zinc-400 py-4">{dropTargetColumn === 'to-do' ? 'Drop here' : 'No requests'}</p>
            ) : (
              <AnimatePresence mode="popLayout">
                {toDoRequests.map((req, idx) => (
                  <div key={req.id} className="relative" {...createCardDropHandlers('to-do', req, idx)}>
                    {dropTargetBeforeId === req.id && <div className="absolute -top-1 left-0 right-0 h-0.5 bg-blue-500 rounded z-10" aria-hidden />}
                    <RequestCard request={req} onClick={setSelectedRequest} priorityStyles={priorityStyles} draggable onDragStart={(e) => handleDragStart(e, req)} ticketPrefix="FAC" />
                  </div>
                ))}
              </AnimatePresence>
            )}
        ...
        <div
          className={`p-4 flex flex-col flex-1 min-h-[160px] rounded-lg border-2 border-dashed transition-colors ${
            dropTargetColumn === 'in-progress' ? 'border-blue-500 bg-blue-500/10 dark:bg-blue-500/20' : 'border-transparent'
          }`}
          {...inProgressDropHandlers}
        >
          <h3 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3">
            In Progress
            <span className="ml-2 text-zinc-700 dark:text-zinc-300 font-normal">{inProgressRequests.length}</span>
          </h3>
          <div className="flex-1 space-y-2 overflow-y-auto">
            {inProgressRequests.length === 0 ? (
              <p className="text-sm text-zinc-500 dark:text-zinc-400 py-4">{dropTargetColumn === 'in-progress' ? 'Drop here' : 'No requests'}</p>
            ) : (
              <AnimatePresence mode="popLayout">
                {inProgressRequests.map((req, idx) => (
                  <div key={req.id} className="relative" {...createCardDropHandlers('in-progress', req, idx)}>
                    {dropTargetBeforeId === req.id && <div className="absolute -top-1 left-0 right-0 h-0.5 bg-blue-500 rounded z-10" aria-hidden />}
                    <RequestCard request={req} onClick={setSelectedRequest} priorityStyles={priorityStyles} draggable onDragStart={(e) => handleDragStart(e, req)} ticketPrefix="FAC" />
                  </div>
                ))}
      ...
      <DrawerModal isOpen={!!selectedRequest} onClose={() => { setSelectedRequest(null); setSafetyModeComplete(false) }} title={selectedRequest?.title ?? 'Request details'}>
        {selectedRequest && (
          <>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4">
        <div>
          <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Request</p>
          <p className="text-zinc-900 dark:text-zinc-100 mt-1">{request.title}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Submitted by</p>
            <p className="text-zinc-700 dark:text-zinc-300 mt-1">{request.submittedBy ?? '—'}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Time</p>
          ...
          <div>
            ...
            <p className="mt-1">
              <span className={`inline-flex px-2.5 py-1 rounded-md text-xs font-medium border ${priorityClass}`}>
                {request.priority === 'critical' ? 'Critical' : 'Normal'}
              </span>
            </p>
          </div>
          {request.assignedTo && (
            <div>
              <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Assigned to</p>
      ...
      {isNew && (
        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-700">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-2">This will assign the request to you.</p>
        </div>
      )}
      {isInProgress && (
        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-700">
