    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-lg font-semibold text-slate-900 truncate">{batch.name}</h2>
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {batch.school && (
              <span className="text-xs text-slate-400">{batch.school.name}</span>
            )}
            {batch.schoolYear && (
              <span className="text-xs text-slate-400">{batch.schoolYear}</span>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          {isDraft && (
            <>
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
          ...
          {!isDeployment && onOpenDamageReport && (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 active:scale-[0.97] transition-all ml-auto"
            >
      ...
      {!isActive && !isDeployment && onOpenDamageReport && (
        <div className="flex">
          <button
            onClick={() => onOpenDamageReport(batchId)}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 active:scale-[0.97] transition-all"
          >
            <FileWarning className="w-4 h-4" />
            View Damage Report
          </button>
      ...
      {batch.items.length === 0 ? (
        <div className="ui-glass p-8 text-center">
          <p className="text-sm text-slate-500">No devices in this batch</p>
                        ...
                        {assigningItemId === item.id ? (
                          <div className="space-y-1.5 min-w-[200px]">
                            ...
                            {studentResults.length > 0 && (
                              <div className="max-h-28 overflow-y-auto rounded-md border border-slate-200 bg-white">
                                {studentResults.map((s) => (
                                  <button
                                    key={s.id}
                                    onClick={() => assignStudentMutation.mutate({ itemId: item.id, studentId: s.id })}
                                    disabled={assignStudentMutation.isPending}
                                    className="w-full text-left px-2.5 py-1.5 text-xs hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0 cursor-pointer"
                                  >
                                    <span className="font-medium text-slate-900">
                                      {s.firstName} {s.lastName}
                                    </span>
                                    {s.studentId && (
                                      <span className="ml-1.5 text-slate-400 font-mono">#{s.studentId}</span>
                                    )}
                                  </button>
                                ))}
                              </div>
                        ...
                        <td className="px-4 py-3 text-right">
                          {item.status === 'PENDING' ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900 text-white text-xs font-medium hover:bg-slate-800 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
                            >
              ...
              <div key={item.id} className="ui-glass p-4 space-y-2">
                ...
                {!isDeployment && item.condition && (
                  <div className="flex items-center gap-2">
                    ...
                    {item.damageFeeCents != null && item.damageFeeCents > 0 && (
                      <span className="text-xs text-slate-600">${(item.damageFeeCents / 100).toFixed(2)}</span>
                    )}
                  </div>
                )}
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleProcessItem(item)}
                        disabled={processItemMutation.isPending}
                        className="px-3 py-1.5 rounded-full bg-slate-900 text-white text-xs font-medium hover:bg-slate-800 active:scale-[0.97] disabled:opacity-50 cursor-pointer"
                      >
                        Process
                      </button>
                    </div>
                ...
                {isDeployment && isActive && canProcess && item.status === 'PENDING' && (
                  <button
                    onClick={() => handleProcessItem(item)}
                    disabled={processItemMutation.isPending}
                    className="w-full mt-1 px-3 py-1.5 rounded-full bg-slate-900 text-white text-xs font-medium hover:bg-slate-800 active:scale-[0.97] disabled:opacity-50 cursor-pointer"
                  >
                    Process
                  </button>
                )}
