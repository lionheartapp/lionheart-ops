      ...
      <div className="space-y-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="ui-glass p-5 flex items-start gap-4">
            <div className="w-3 h-3 rounded-full bg-slate-200 animate-pulse mt-1.5" />
    ...
    >
        ...
        <div className="flex gap-2">
          {canManage && (
            <button
              ...
              className="px-5 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 active:scale-[0.97] transition-colors duration-200 disabled:opacity-50"
            >
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-900">E-Rate Compliance Progress</h3>
          <span className="text-sm text-slate-600">
            <AnimatedCounter value={completedCount} className="font-semibold text-slate-900" />
        ...
        <p className="text-xs text-slate-500 mt-2">{progressPct}% complete for {schoolYear}</p>
      </motion.div>
      ...
      <motion.div variants={fadeInUp} className="space-y-3">
        ...
        {tasks.length === 0 && !calendarLoading && (
          <div className="ui-glass p-8 text-center">
            ...
            <p className="text-sm font-medium text-slate-600 mb-1">No calendar tasks for {schoolYear}.</p>
            {canManage && (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="pt-1">
                  {task.status === 'COMPLETED' ? (
                    ...
                    <span
                      className={`block w-3 h-3 rounded-full ${STATUS_DOT[task.status] || 'bg-slate-400'}`}
                    />
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {task.documentCount > 0 && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-xs font-medium shrink-0">
                      ...
                      {isCompleting ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            rows={2}
                            className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white resize-none focus:outline-none focus:ring-2 focus:ring-blue-400/40"
                          />
                          <div className="flex gap-2">
                            <button
                            ...
                            <button
                              ...
                              className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-50 active:scale-[0.97] transition-colors duration-200"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <button
                          ...
                          className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-slate-700 text-xs font-medium hover:bg-slate-50 active:scale-[0.97] transition-colors duration-200"
                        >
