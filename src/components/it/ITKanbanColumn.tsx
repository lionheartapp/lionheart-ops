...
export default function ITKanbanColumn({ status, tickets, onTicketClick }: ITKanbanColumnProps) {
  ...
  const statusInfo = STATUS_COLORS[status] ?? { bg: 'bg-gray-100', text: 'text-gray-700', label: status }

  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col rounded-2xl bg-gray-50/80 border transition-colors ${
        isOver ? 'border-blue-300 bg-blue-50/30' : 'border-gray-200/50'
      }`}
    >
      {/* Column header */}
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400 font-medium">{tickets.length}</span>
        </div>
      </div>
