      ...
      <div className="ui-glass-table">
        ...
        {[...Array(5)].map((_, i) => (
          <div key={i} className="flex items-center gap-4 px-4 py-3 border-b border-gray-100 last:border-0">
            ...
            {[...Array(3)].map((_, j) => (
              <div key={j} className="h-8 w-16 bg-gray-100 rounded-lg mx-auto" />
            ))}
          </div>
        ))}
      </div>
...
function MatrixEmpty({ eventProjectId }: { eventProjectId: string }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-base font-semibold text-gray-900 mb-2">No document requirements yet</h3>
      ...
      <td className="px-4 py-2.5 sticky left-0 bg-white z-10 min-w-[160px]">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate max-w-[100px]">{fullName}</p>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              <strong className="text-gray-900">{completeCount}</strong> of{' '}
              <strong className="text-gray-900">{allParticipants.length}</strong> fully complete
            </span>
          </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[180px] max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-300 transition-colors"
          />
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/80">
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide sticky left-0 bg-gray-50/80 z-10 min-w-[160px]">
                Participant
              </th>
              {requirements.map((req) => (
                <th
                  key={req.id}
                  className="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wide max-w-[100px]"
                  title={req.label}
                >
