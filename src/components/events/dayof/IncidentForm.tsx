...
export default function IncidentForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 px-4 py-5">
      ...
      {/* Incident type */}
      <fieldset>
        <legend className="text-sm font-semibold text-gray-700 mb-2">Incident Type</legend>
          ...
          {INCIDENT_TYPES.map(({ value, label, Icon, color, selectedColor }) => (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 p-2.5 rounded-xl border text-xs font-medium transition-all cursor-pointer ${
                type === value ? selectedColor : `${color} hover:bg-gray-50`
              }`}
            >
      ...
      {/* Severity */}
      <fieldset>
        <legend className="text-sm font-semibold text-gray-700 mb-2">Severity</legend>
          ...
          {SEVERITY_OPTIONS.map(({ value, label, color, selectedColor, dot }) => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-all cursor-pointer ${
                severity === value ? selectedColor : `${color} hover:bg-gray-50`
              }`}
            >
      ...
      <div>
        ...
        <div className="relative">
          <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden bg-white focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400">
            <input
              type="text"
          ...
          {showParticipantDropdown && filteredParticipants.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-auto max-h-48">
              {filteredParticipants.slice(0, 10).map((p) => (
                <button
                  key={p.registrationId}
                  type="button"
                  onClick={() => addParticipant(p.registrationId)}
                  className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 cursor-pointer first:rounded-t-xl last:rounded-b-xl"
                >
                  {p.firstName} {p.lastName}
                  {p.grade && <span className="text-gray-400 ml-1">({p.grade})</span>}
                </button>
              ))}
            </div>
          )}
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
        />
      </div>
      ...
      <div>
        ...
        {followUpNeeded && (
          <textarea
            ...
            className="mt-2 w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
          />
        )}
      </div>
      ...
      <button
        ...
        className="w-full py-3 rounded-xl bg-gray-900 text-white text-sm font-medium hover:bg-gray-800 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
      >
        {isSubmitting ? (
