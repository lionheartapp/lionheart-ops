  ...
  return (
    <div className="space-y-6">
      ...
      <InteractiveCampusMap
        ...
        onOrgCenterChange={async (lat, lng) => { try { const res = await fetch('/api/settings/campus/map-data', { method: 'PATCH', headers: getAuthHeaders(), credentials: 'include' as const, body: JSON.stringify({ latitude: lat, longitude: lng, campusId: selectedCampusId || undefined }) }); if (res.ok) { setMapCenter((prev) => prev ? { ...prev, lat, lng } : prev); setSuccessMessage('Campus position updated'); setTimeout(() => setSuccessMessage(''), 3000) } } catch { setError('Failed to save campus position') } }}
