...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompt = `Analyze this school or organization website HTML and extract brand colors and logo.

Return a JSON object with these keys (use null for anything you cannot confidently determine):
- primaryColor: hex color like "#1a3a5c" (main brand color from headers, buttons, or theme)
- secondaryColor: hex color like "#c4a006" (accent color if present)
- logoUrl: absolute URL to the main logo image (from img src, og:image, or link[rel="icon"])

Rules:
- Prefer theme-color, meta og:image, and prominent CSS colors
- If you find only one strong color, use it as primary and leave secondary null
